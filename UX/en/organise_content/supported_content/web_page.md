...
| `actions.manipulation.location`   | `shortcut` or undefined   | Controls where the manipulation invite appears when manipulation mode is set to `toggle`. Set to `shortcut` to display in the bottom right corner, leave undefined for the default centered button |
